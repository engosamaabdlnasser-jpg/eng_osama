...
export default function AdminCourse() {
  ...
  const [title, setTitle] = useState(''); const [titleEn,setTitleEn]=useState(''); const [description, setDescription] = useState(''); const [descriptionEn,setDescriptionEn]=useState(''); const [instructor, setInstructor] = useState('ENG OSAMA'); const [instructorEn,setInstructorEn]=useState('ENG OSAMA'); const [ltitleEn,setLtitleEn]=useState(''); const [ldescEn,setLdescEn]=useState(''); const [category, setCategory] = useState(''); const [image, setImage] = useState(''); const [published, setPublished] = useState(false); const [lessons, setLessons] = useState<Lesson[]>([]);
  ...
  useEffect(() => { (async () => {
    ...
    if (edit && id) { const c = await getCourse(id); if (!c) { setError('الكورس غير موجود.'); return; } setCourse(c); setTitle(c.title_ar||c.title); setTitleEn(c.title_en||''); setDescription(c.description_ar||c.description); setDescriptionEn(c.description_en||''); setInstructor(c.instructor_name_ar||c.instructor_name); setInstructorEn(c.instructor_name_en||''); setCategory(c.category_id ?? ''); setImage(c.image_url ?? ''); setPublished(c.published); setLessons([...(c.lessons ?? [])].sort((a,b) => a.sort_order - b.sort_order)); }
  ...
  async function save(e: React.FormEvent) { e.preventDefault(); setError(''); setMsg(''); if (!supabase) return; setBusy(true); try {
    const payload = { title: title.trim(), title_ar:title.trim(), title_en:titleEn.trim()||null, description: description.trim(), description_ar:description.trim(), description_en:descriptionEn.trim()||null, instructor_name: instructor.trim() || 'ENG OSAMA', instructor_name_ar:instructor.trim()||'ENG OSAMA', instructor_name_en:instructorEn.trim()||'ENG OSAMA', image_url: image.trim() || null, category_id: category || null, published };
  ...
  async function addLesson(e: React.FormEvent) { e.preventDefault(); setError(''); if (!supabase || !course) return; const video = extractYouTubeVideoId(lurl); if (!video) { setError('رابط YouTube غير صالح. استخدم رابط watch أو youtu.be أو embed.'); return; } setBusy(true); try {
    const { data, error } = await supabase.from('lessons').insert({ course_id: course.id, title: ltitle.trim(), title_ar:ltitle.trim(), title_en:ltitleEn.trim()||null, description: ldesc.trim(), description_ar:ldesc.trim(), description_en:ldescEn.trim()||null, youtube_url: lurl.trim(), sort_order: lessons.length + 1 }).select().single(); if (error) throw error;
    setLessons(xs => [...xs, data as Lesson]); setLtitle(''); setLtitleEn(''); setLurl(''); setLdesc(''); setLdescEn(''); setMsg('تم إضافة الدرس بنجاح.');
  ...
  return <section className="section"><div className="container" style={{maxWidth:900}}><Link className="muted" to="/admin">← لوحة التحكم</Link><h1 className="section-title" style={{marginTop:14}}>{edit ? 'تعديل الكورس' : 'إضافة كورس'}</h1>
    <form className="surface form-grid" style={{padding:24}} onSubmit={save}>
      <div className="two-col"><div><label className="label">العنوان AR</label><input className="input" required value={title} onChange={e=>setTitle(e.target.value)}/></div><div><label className="label">Title EN</label><input className="input" value={titleEn} onChange={e=>setTitleEn(e.target.value)}/></div></div>
      <div className="two-col"><div><label className="label">الوصف AR</label><textarea className="input" required rows={5} value={description} onChange={e=>setDescription(e.target.value)}/></div><div><label className="label">Description EN</label><textarea className="input" rows={5} value={descriptionEn} onChange={e=>setDescriptionEn(e.target.value)}/></div></div>
      <div className="two-col"><div><label className="label">التصنيف</label><select className="input" value={category} onChange={e=>setCategory(e.target.value)}><option value="">بدون تصنيف</option>{cats.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div><label className="label">المدرب AR</label><input className="input" value={instructor} onChange={e=>setInstructor(e.target.value)}/></div><div><label className="label">Instructor EN</label><input className="input" value={instructorEn} onChange={e=>setInstructorEn(e.target.value)}/></div></div>
