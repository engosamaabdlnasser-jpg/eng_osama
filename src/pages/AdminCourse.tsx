...
export default function AdminCourse() {
  const { id } = useParams(); const edit = Boolean(id); const nav = useNavigate(); const { locale, t } = useI18n();
  const [cats, setCats] = useState<Category[]>([]); const [course, setCourse] = useState<Course | null>(null);
  const [title, setTitle] = useState(''); const [titleAr, setTitleAr] = useState(''); const [titleEn, setTitleEn] = useState(''); const [description, setDescription] = useState(''); const [descriptionAr, setDescriptionAr] = useState(''); const [descriptionEn, setDescriptionEn] = useState(''); const [instructor, setInstructor] = useState('ENG OSAMA'); const [category, setCategory] = useState(''); const [image, setImage] = useState(''); const [published, setPublished] = useState(false); const [lessons, setLessons] = useState<Lesson[]>([]);
  const [ltitleAr, setLtitleAr] = useState(''); const [ltitleEn, setLtitleEn] = useState(''); const [ldescAr, setLdescAr] = useState(''); const [ldescEn, setLdescEn] = useState(''); const [imageBusy, setImageBusy] = useState(false); const [ltitle, setLtitle] = useState(''); const [lurl, setLurl] = useState(''); const [ldesc, setLdesc] = useState(''); const [error, setError] = useState(''); const [msg, setMsg] = useState(''); const [busy, setBusy] = useState(false);
  ...
  useEffect(() => { (async () => {
    ...
    if (edit && id) { const c = await getCourse(id); if (!c) { setError('الكورس غير موجود.'); return; } setCourse(c); setTitle(c.title); setTitleAr(c.title_ar || c.title); setTitleEn(c.title_en || ''); setDescription(c.description); setDescriptionAr(c.description_ar || c.description); setDescriptionEn(c.description_en || ''); setInstructor(c.instructor_name); setCategory(c.category_id ?? ''); setImage(c.image_url ?? ''); setPublished(c.published); setLessons([...(c.lessons ?? [])].sort((a,b) => a.sort_order - b.sort_order)); }
  ...
  async function save(e: React.FormEvent) { e.preventDefault(); setError(''); setMsg(''); if (!supabase) return; setBusy(true); try {
    const payload = { title: title.trim(), title_ar: titleAr.trim() || title.trim(), title_en: titleEn.trim() || title.trim(), description: description.trim(), description_ar: descriptionAr.trim() || description.trim(), description_en: descriptionEn.trim() || description.trim(), image_url: image.trim() || null, category_id: category || null, instructor_name: instructor.trim() || 'ENG OSAMA', published };
  ...
  async function addLesson(e: React.FormEvent) { e.preventDefault(); setError(''); if (!supabase || !course) return; const video = extractYouTubeVideoId(lurl); if (!video) { setError('رابط YouTube غير صالح. استخدم رابط watch أو youtu.be أو embed.'); return; } setBusy(true); try {
    const { data, error } = await supabase.from('lessons').insert({ course_id: course.id, title: ltitle.trim(), title_ar: ltitleAr.trim() || ltitle.trim(), title_en: ltitleEn.trim() || ltitle.trim(), description: ldesc.trim(), description_ar: ldescAr.trim() || ldesc.trim(), description_en: ldescEn.trim() || ldesc.trim(), youtube_url: lurl.trim(), sort_order: lessons.length + 1 }).select().single(); if (error) throw error;
    setLessons(xs => [...xs, data as Lesson]); setLtitle(''); setLtitleAr(''); setLtitleEn(''); setLurl(''); setLdesc(''); setLdescAr(''); setLdescEn(''); setMsg('تم إضافة الدرس بنجاح.');
  ...
  return <section className="section"><div className="container" style={{maxWidth:900}}><Link className="muted" to="/admin">← لوحة التحكم</Link><h1 className="section-title" style={{marginTop:14}}>{edit ? 'تعديل الكورس' : 'إضافة كورس'}</h1>
    <form className="surface form-grid" style={{padding:24}} onSubmit={save}>
      <div className="two-col"><div><label className="label">{locale==='ar'?'العنوان بالعربية':'Arabic title'}</label><input className="input" required value={titleAr} onChange={e=>{setTitleAr(e.target.value);setTitle(e.target.value)}} /></div><div><label className="label">{locale==='ar'?'العنوان بالإنجليزية':'English title'}</label><input className="input" value={titleEn} onChange={e=>setTitleEn(e.target.value)} /></div></div>
      <div className="two-col"><div><label className="label">{locale==='ar'?'الوصف بالعربية':'Arabic description'}</label><textarea className="input" required rows={5} value={descriptionAr} onChange={e=>{setDescriptionAr(e.target.value);setDescription(e.target.value)}} /></div><div><label className="label">{locale==='ar'?'الوصف بالإنجليزية':'English description'}</label><textarea className="input" rows={5} value={descriptionEn} onChange={e=>setDescriptionEn(e.target.value)} /></div></div>
    ...
    {edit && course && <div style={{marginTop:28}}><h2 className="section-title">الدروس</h2><form className="surface form-grid" style={{padding:20}} onSubmit={addLesson}><div className="two-col"><div><label className="label">{locale==='ar'?'عنوان الدرس بالعربية':'Arabic lesson title'}</label><input className="input" required value={ltitleAr} onChange={e=>{setLtitleAr(e.target.value);setLtitle(e.target.value)}} /></div><div><label className="label">{locale==='ar'?'عنوان الدرس بالإنجليزية':'English lesson title'}</label><input className="input" value={ltitleEn} onChange={e=>setLtitleEn(e.target.value)} /></div></div><div className="two-col"><div><label className="label">{locale==='ar'?'الوصف بالعربية':'Arabic description'}</label><textarea className="input" rows={3} value={ldescAr} onChange={e=>{setLdescAr(e.target.value);setLdesc(e.target.value)}} /></div><div><label className="label">{locale==='ar'?'الوصف بالإنجليزية':'English description'}</label><textarea className="input" rows={3} value={ldescEn} onChange={e=>setLdescEn(e.target.value)} /></div></div><div><label className="label">YouTube URL</label><input className="input" required type="url" value={lurl} onChange={e=>setLurl(e.target.value)} placeholder="https://www.youtube.com/watch?v=..."/></div><button className="btn btn-primary" disabled={busy}>{busy ? 'جاري الإضافة...' : 'إضافة الدرس'}</button></form><div className="grid" style={{marginTop:14}}>{lessons.map((l,i)=><div className="surface" key={l.id} style={{padding:16,display:'flex',gap:12,alignItems:'center'}}><strong>{i+1}. {l.title}</strong><span className="muted" style={{flex:1}}>{extractYouTubeVideoId(l.youtube_url) ? 'YouTube صالح' : 'رابط غير صالح'}</span><button className="btn btn-danger" onClick={()=>removeLesson(l.id)}>حذف</button></div>)}</div></div>}
  </div></section>;
}
