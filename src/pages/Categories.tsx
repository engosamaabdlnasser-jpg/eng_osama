...
export default function Categories(){
  ...
  useEffect(()=>{let active=true;Promise.all([getCategories(),getSiteSettings()]).then(([c,site])=>{if(!active)return;setCats(c);setSettings(site)}).catch(e=>active&&setError(e instanceof Error?e.message:'تعذر تحميل التصنيفات.')).finally(()=>active&&setLoading(false));return()=>{active=false}},[]);
