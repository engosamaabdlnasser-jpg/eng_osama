...
import { getCategories, getCourses, getSiteSettings } from '../services/data';
...
export default function Home(){
  ...
  useEffect(()=>{let active=true;Promise.all([getCourses(),getCategories(),getSiteSettings()]).then(([c,k,s])=>{if(!active)return;setCourses(c);setCats(k);setSettings(s)}).catch(e=>active&&setError(e instanceof Error?e.message:'تعذر تحميل الصفحة الرئيسية.')).finally(()=>active&&setLoading(false));return()=>{active=false}},[]);
  const s = settings;
